...
const Calls = () => {
    ...
    useEffect(() => {
        axios.get('/activities')
            .then(res => {
              setCalls(res.data.filter(call => call.is_archived === false));
              setLoading(false);
            })
            .catch(err => {
                console.log(err);
    ...
    const handleClick = (event,id) => {
      ...
      switch (event.detail) {
        case 2: {
          axios.post(`/activities/${id}`, {is_archived: true}).then(res => {
            alert("Call archived");
            axios.get('/activities')
            .then(res => {
              setCalls(res.data.filter(call => call.is_archived === false));
  ...
  return (
    <div className='continer' style={{overflow: "hidden"}}>
      ...
      {calls.length === 0  && !loading? <p style={{textAlign: "center", marginTop: 30, color: "green"}}>No calls</p> : null}
    </div>
  );
};
